...
// ----------------------------------------------------------------------------
	...
	if (strcmp(name, "UART")==0){
		...
		UART0_REG(UART_DIV) = CPU_FREQ/2/115200-1;
		UART0_REG(UART_TXCTRL) = 0x1;
		UART0_REG(UART_RXCTRL) = 0x1;
...
// ----------------------------------------------------------------------------
	...
	if (isatty(file)) {
		...
		while( count<len && ((rxfifo = UART0_REG(UART_RXFIFO)) >0) ){
			*ptr++ = (char)rxfifo;
			count++;
		}
...
// ----------------------------------------------------------------------------
	...
	if (isatty(file)) {
		...
		for (size_t i = 0; i < len; i++) {
			...
			while (UART0_REG(UART_TXFIFO) & 0x80000000){;}

			UART0_REG(UART_TXFIFO) = buff[i];

			if (buff[i] == '\n') {
				while (UART0_REG(UART_TXFIFO) & 0x80000000){;}
				UART0_REG(UART_TXFIFO) = '\r';
			}
